...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    // console.log('clicou');
    saveCartID(id);
    // const botão = document.querySelectorAll('.product_add');
    const produtoPeloId = await fetchProduct(id);
    const saveLi = createCartProductElement(produtoPeloId);
    const cartProduct = document.querySelector('.cart__products');
    cartProduct.appendChild(saveLi);
  });
